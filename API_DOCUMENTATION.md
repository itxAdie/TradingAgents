...
## Part E — Phase 4 HTTP/SSE Interfaces (IMPLEMENTED)

Served by `tradingagents.api.app.create_app(ServerSettings)`; same surface
as `tradingagents serve`. All responses JSON unless noted. Errors use the
envelope `{"error": {"code", "message", "details?"}}`; FastAPI validation
errors are normalized to code `validation_error` with the detail list.

| Endpoint | Method | Notes |
|---|---|---|
| /api/health | GET | liveness |
| /api/system/status | GET | `{overall, components[], generated_at}` |
| /api/system/settings | GET | read-only settings echo (no secrets) |
| /api/system/audit | GET | paginated audit rows `{ts, action, detail}` |
| /api/markets | GET | bare array: spec/quote/change/freshness/note |
| /api/markets/{asset}/candles | GET | `timeframe`, `limit`; CandleOut(open/high/low/close/volume) + data_status/freshness |
| /api/markets/{asset}/indicators | GET | `kinds=sma20,ema50,bb20` CSV → aligned series w/ nulls + na_reasons |
| /api/research | GET /api/research/{id} GET | run index/detail; artifacts verbatim |
| /api/signals | GET | filters: state, asset, min_confidence; envelope |
| /api/signals/{id} | GET | record + transitions + orders + research attribution |
| /api/portfolio | GET | PAPER_ACCOUNT_REPORT verbatim incl. disclaimer |
| /api/portfolio/equity-curve | GET | chronological points (timestamp/equity/cash/exposure/drawdown_pct) |
| /api/trades | GET | list items (`outcome`, `has_journal`); filter `outcome=win\|loss` |
| /api/trades/{id} | GET | detail + timeline stages + journal |
| /api/trades/{id}/journal | PUT | body `{text, author}` — the only write endpoint besides halt/resume; audited |
| /api/risk | GET | limits with utilization_pct + halted flag |
| /api/risk/events | GET | risk decision/event log |
| /api/backtests | POST | submit `{asset_id, timeframe, start, end, include_walk_forward}` → 202 job |
| /api/backtests | GET /{run_id} GET | registry + `{job, report?}`; report.strategies[].stats{}, report.walk_forward[] per-strategy aggregates with windows[] |
| /api/events/stream | GET (SSE) | named events (see sse.ts KNOWN_EVENTS); heartbeat comments; Last-Event-ID replay |

SPA fallback: non-/api GET/HEAD 404s return dashboard/dist/index.html when
`dashboard_dir` is configured. Server binds 127.0.0.1; no auth in Phase 4.
