...
# PHASE 4 — Web Terminal & Dashboard (**PROPOSED — awaiting user approval; nothing implemented yet**)

## P4.0 Core principle — presentation layer only

> **Status: implemented** (`tradingagents/api/` + `dashboard/`; see
> SESSION_LOG.md Entry 5 for delivery notes and test evidence).

The dashboard is a visualization/monitoring surface over existing engines.
The frontend contains ZERO trading logic: it never computes P&L, risk,
position size, drawdown or signals; it renders backend-computed values
verbatim (including literal disclaimers). The backend remains the single
source of truth. No broker integration is introduced (Phase 5 concern);
the UI labels every execution context `PAPER TRADING — SIMULATED
EXECUTION ONLY` and is architected so a future `LIVE` environment would be
a distinct, explicitly-labelled mode.

## P4.1 Backend API layer — new additive package `tradingagents/api/`

FastAPI application served by uvicorn, mounted as a flat CLI command
(`cli/api.py` → `register_api_command(app)` → `tradingagents serve`,
matching the existing registration convention at cli/main.py:74-80).
Binds `127.0.0.1` by default; serves the built frontend from
`dashboard/dist` plus JSON APIs under `/api/*`.

Response contracts are Pydantic v2 models (`api/schemas.py`) derived from
existing domain models — no ad-hoc dicts. Standard list envelope:
`{items, total, limit, offset}` with server-side filtering/sorting;
standard error envelope `{error: {code, message, detail}}`. Canonical
UTC ISO-8601 timestamps everywhere.

Endpoints (wrap existing capabilities — never re-implement):

```
GET /api/markets                          registry assets + quote freshness
GET /api/markets/{asset}                  detail panel data (quote + latest signal/report refs)
GET /api/markets/{asset}/candles          timeframe/start/end/limit → OhlcvSeries bars
GET /api/markets/{asset}/indicators       EMA/SMA/Bollinger arrays via analysis/indicators.py
GET /api/research                         paginated run index (new artifact store, P4.3)
GET /api/research/{run_id}                full ResearchReport JSON (sections incl. bull/bear/verdict)
GET /api/signals                          filters: asset/action/timeframe/status/date/confidence
GET /api/signals/{signal_id}              PaperSignalRecord + transitions + order events
GET /api/portfolio                        account_summary() verbatim
GET /api/portfolio/equity-curve           EquitySnapshot rows
GET /api/portfolio/daily                  DailyPerformanceRow rows (folded)
GET /api/portfolio/positions              PositionRecord rows with marks
GET /api/trades                           TradeRecord rows + journal presence, filters
GET /api/trades/{trade_id}                full lifecycle (order events folded + signal snapshot + journal)
PUT /api/trades/{trade_id}/journal        add note (audited mutation)
GET /api/risk                             configured limits vs current utilization (risk.py values)
GET /api/risk/events                      risk/halt/rejected-signal events from store history
GET /api/backtests                        saved BacktestReport index
POST /api/backtests                       start run (background thread; audited mutation)
GET /api/backtests/{run_id}               BacktestReport JSON (stats/ledger/walk-forward/equity)
GET /api/system/status                    component status matrix
GET /api/events/stream                    SSE realtime event stream (P4.4)
```

## P4.2 Reuse map (backend additions stay thin)

| Dashboard need | Existing source consumed unchanged |
|---|---|
| Candles/quotes | `MarketDataProvider.get_ohlcv/get_quote` (Yahoo provider) |
| Indicator overlays | `analysis/indicators.py` ground-truth engine |
| Research reports/debate | `ResearchReport`/`ResearchSignal` schemas; paper engine's embedded snapshots |
| Signals + transitions | `JsonPaperStateStore` records + `signals_index.jsonl` |
| Portfolio/positions/P&L | `PaperTradingEngine.account_summary()`, `build_report()`, equity/daily JSONL |
| Trade lifecycle | `fold_order_events(load_order_events())` + journal entries |
| Risk limits/utilization | `paper/risk.py` outputs already persisted by engine |
| Backtest results/walk-forward | `BacktestReport.to_json()` artifacts |
| Scheduler state | `scheduler.json` per-slot rows |
| Engine lifecycle events | existing `NotificationProvider` protocol seam |

## P4.3 Small new backend capabilities (additive, no engine changes)

1. **ResearchArtifactStore** — persist `AssembledResult` report+signal JSON per
   research run under `{data_cache_dir}/research_runs/{asset}_{tf}/{ts}.json`
   plus an append-only index for listing/pagination. Wired by wrapping the
   RunnerProtocol object handed to `PaperTradingEngine` (and used directly by
   the research route) — zero changes inside the engine.
2. **Read-only query helpers** on `JsonPaperStateStore`: `list_signals(filters)`
   (reads `signals/*.json`), plus thin pagination wrappers over existing
   full-collection loads (trades/orders/equity/daily/journal).
3. **BacktestRunRegistry** — `POST /api/backtests` runs `backtest.run_backtest`
   (+ optional walk-forward) in a worker thread using the same CLI wiring,
   saves `BacktestReport` to `{cache}/backtests/{run_id}.json`, emits
   BACKTEST_COMPLETED on the bus. One concurrent run; queue depth 1.
4. **EventBus** — in-process pub/sub implementing `NotificationProvider`;
   the paper engine's `_emit` calls flow to SSE subscribers untouched. Also
   carries RESEARCH_*, BACKTEST_* and audit events from the API layer.
5. **Audit log** — every state-changing API call (start backtest, journal
   note, halt/resume, settings change) appends `{ts, actor, action, args_hash}`
   to `{cache}/audit.jsonl` and emits an event. Mutating routes are explicit
   and few; everything else is GET-only.

## P4.4 Real-time strategy — SSE + controlled polling

`GET /api/events/stream` (Server-Sent Events) streams typed JSON events:
PRICE_UPDATED, SIGNAL_GENERATED/REJECTED/ACCEPTED, POSITION_OPENED/UPDATED/
CLOSED, RISK_EVENT, RESEARCH_STARTED/COMPLETED, BACKTEST_COMPLETED,
SYSTEM_STATUS. SSE chosen over WebSocket: all listed flows are
server→client, reconnect semantics are trivial, no extra protocol layer.
Frontend shows LIVE / RECONNECTING / OFFLINE connection state driven by the
stream health. Market price ticks come from client polling of
`/api/markets/{asset}` at a conservative interval (default 30 s, only while
the Markets/Dashboard view is visible); AI research content updates ONLY via
engine/scheduler events — never on price polls. Freshness labels ("updated Ns
ago", data `status` field) rendered everywhere stale data could mislead.

## P4.5 Frontend — new `dashboard/` workspace (no prior frontend exists)

Stack: React 18 + TypeScript + Vite, Tailwind CSS (dark-first terminal
theme), TanStack Query v5 for ALL server state (caching/polling/
invalidation), react-router v7 with asset/timeframe encoded in URL params
(UI state is URL state), TradingView `lightweight-charts` v4 for
candlestick/equity/drawdown/exposure charts, plain accessible HTML tables
(no heavy grid). Vitest + Testing Library for components; Playwright for
E2E. Dev proxy to uvicorn; production build copied/served by the API.

Pages: Dashboard Home · Markets (XAUUSD/BTCUSD × 15m/1h/4h/1d) · Market
Detail · Research list/detail (Executive Summary, Technical, Macro, News,
Sentiment, Bull Case, Bear Case, Consensus, Risks, Signal, Invalidation,
Data Sources, AI Models sections mapped 1:1 from `ResearchReport` fields)
· Debate view (Bull vs Bear cards → Manager verdict, structured fields
only; agent pipeline timeline Technical→News→Sentiment→Bull→Bear→Manager→
Signal with statuses/failures/models; NO chain-of-thought — only the
structured outputs the schema exposes) · Signals list/detail · Portfolio ·
Positions/detail · Journal/detail · Backtests list/configure/result · Risk
(limits vs utilization bars + event feed) · System Status · Settings.
Persistent header badge: `PAPER TRADING — SIMULATED EXECUTION ONLY`.
Design language: information-dense, dark, minimal decoration; BUY/SELL/
profit/loss/status always carry text/icon cues, not color alone.

## P4.6 Security & authentication

No authentication in Phase 4 (single-user local tool): server binds
127.0.0.1 only. Optional shared-token guard via env
(`TRADINGAGENTS_API_TOKEN` → Bearer check) designed so multi-user/RBAC can
be layered later without route changes. Secrets live exclusively in backend
env vars; no key material can appear in any response schema (verified:
domain models carry model-id strings and hashes only). CORS restricted to
the Vite dev origin; same-origin in production. Frontend cannot execute
trades: no trade-execution endpoint exists at all — the only mutations are
audited backtest-start/journal-note/settings actions.

## P4.7 Testing plan (details go to TESTING_PLAN.md on approval)

Backend: FastAPI TestClient suites per route group (happy path, filters,
pagination edges, invalid params → 4xx, empty/stale states, SSE stream
shape, auth token when enabled), all offline with fake providers/stores —
mirroring existing test style. Frontend: Vitest component/page tests
(loading/error/empty states, table filtering, chart mount with fixture
data, reconnect banner logic). E2E (Playwright, seeded TEST DATA cache dir
with visible TEST DATA banner): dashboard→chart→research→debate→signal→
portfolio→position→journal→performance flow, and backtest configure→run→
results→equity→walk-forward flow. Visual/responsive checks via Playwright
viewport matrix (desktop/laptop/tablet).

## P4.8 Explicit non-goals (Phase 4)

No broker connectivity, no real-money anything, no WebSocket duplex layer,
no enterprise identity system, no second computation of any analytic in
the browser, no modification of engine internals beyond documented seams
(RunnerProtocol wrapper, NotificationProvider adapter, store read-helpers).
...
| 4 Dashboard | UI over stored runs | **implemented** (`tradingagents/api/`, `dashboard/`, P4.x above; reuses paper store, research artifacts, backtest registry verbatim) |
...
Phase 2 + Phase 3 + Phase 4 implementation; suite 913 passed / 2 skipped /
69 subtests, ruff clean; Playwright E2E 8 passed, Vitest 21 passed).
Update this file
